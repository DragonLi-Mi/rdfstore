...
//---------------------------------------------------------------------------
QueryGraph::Edge::Edge(unsigned long from,unsigned long  to,const vector<unsigned long>& common)
...
QueryGraph::Filter::Filter(const Filter& other)
   : type(other.type),arg1(other.arg1?new Filter(*other.arg1):0),arg2(other.arg2?new Filter(*other.arg2):0),arg3(other.arg3?new Filter(*other.arg3):0),id(other.id),value(other.value),valueType(other.valueType)
   // Copy-Constructor
{
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
bool QueryGraph::Filter::isApplicable(const std::set<unsigned long>& variables) const
   // Could be applied?
{
   // A variable?
   if (type==Variable)
...
//---------------------------------------------------------------------------
static bool intersects(const set<unsigned long>& a,const set<unsigned long>& b,vector<unsigned long>& common)
   // Check if two sets overlap
{
   common.clear();
   set<unsigned long>::const_iterator ia,la,ib,lb;
   if (a.size()<b.size()) {
      if (a.empty())
         return false;
      ia=a.begin(); la=a.end();
...
void QueryGraph::setQueryGraph(QueryGraph::SubQuery& Query)
{
...
//---------------------------------------------------------------------------
static void constructEdges(QueryGraph::SubQuery& subQuery,set<unsigned long>& bindings)
   // Construct the edges for a specfic subquery
{
   // Collect all variable bindings
   vector<set<unsigned long> > patternBindings,optionalBindings,unionBindings,gjoinBindings;
