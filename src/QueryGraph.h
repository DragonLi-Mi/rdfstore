...
class QueryGraph
{
   ...
   struct Edge {
      ...
      /// Common variables
      std::vector<unsigned long> common;

      /// Constructor
      Edge(unsigned long from,unsigned long to,const std::vector<unsigned long>& common);
      /// Destructor
      ~Edge();
   };
   /// A value filter
   struct Filter {
      ...
      /// Could be applied?
      bool isApplicable(const std::set<unsigned long>& variables) const;
   };
   /// A table function
   ...
   SubQuery query;
   /// The projection
   std::vector<unsigned long> projection;
   ...
   /// Construct the edges
   void constructEdges();
   void setQueryGraph(QueryGraph::SubQuery& Query);
   ...
   unsigned long getLimit() const { return limit; }
   /// Known empty result
   ...
   void addProjection(unsigned long id) { projection.push_back(id); }
   /// Iterator over the projection
   typedef std::vector<unsigned long >::const_iterator projection_iterator;
   /// Iterator over the projection
