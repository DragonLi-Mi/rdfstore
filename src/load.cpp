 ...
 //   CommandCreateNodefile(nodetable);
    ...
    for(int i=0; i<parse_triple_num;i++)
        ...
        if (subid==0)
        {
          ...
          subid=add2Node(sub);
           cout<<"sub :"<<_sub<<"  sub id: "<<subid<<"   MD5:"<<inserts<<endl;
            CommandInsert(table,inserts,strlen(inserts)+1, &subid, sizeof(subid));
        }
        else
            {
       ...
       //  For predicate
            ...
            if (preid==0)
        {
          ...
          CommandInsert(table, insertp ,strlen(insertp)+1, &preid, sizeof(preid));
           ...
           if (objid==0)
        {
            ...
            CommandInsert(table, inserto ,strlen(inserto)+1, &objid, sizeof(objid));
